...
def test_resolution_data_quality_codes_always_review():
    ...
    for rc in (
        ...
        assert (
            _severity(rc, market_cap_eur=None, sector_wide=False)
...
def test_funnel_diverts_count_as_resolution_review_and_reconcile():
    ...
    assert (
        summary.stage(Stage.BASIS_GATES).entered
        == summary.stage(Stage.RESOLUTION).remaining
    )
...
def test_diverts_do_not_shift_sector_wide():
    ind = [
        _resolved(f"I{i}", sector="Industrials", basis_reason="gross_margin")
        for i in range(6)
    ]
    nsd = _dq_record("NSD", "NO_RAW_MC")
    ...
    summary, dropouts = build_funnel(
        universe=[r.ticker for r in ind] + ["NSD"],
        basis=basis,
        scored=None,
        score_threshold=4.0,
        crosshits_min_dimensions=2,
...
def test_volume_threshold_decoupled_from_growth():
    ...
    assert (
        _severity(ReasonCode.GATE_VOLUME, market_cap_eur=mc, sector_wide=False)
        == SeverityBucket.REVIEW
    )
    assert (
        _severity(ReasonCode.GATE_REVENUE_GROWTH, market_cap_eur=mc, sector_wide=False)
...
def test_large_cap_volume_drop_is_review_regardless_of_metric():
    ...
    assert (
        _severity(
            ...
            market_cap_eur=LARGE_CAP_VOLUME_EUR + 1,
            sector_wide=False,
        )
        == SeverityBucket.REVIEW
    )
...
def test_growth_review_above_growth_threshold():
    assert (
        _severity(
            ReasonCode.GATE_REVENUE_GROWTH,
            market_cap_eur=20_000_000_000,
            sector_wide=False,
        )
        == SeverityBucket.REVIEW
    )
...
def test_market_cap_none_is_benign_never_crashes():
    assert (
        _severity(ReasonCode.GATE_VOLUME, market_cap_eur=None, sector_wide=False)
        == SeverityBucket.BENIGN
    )
...
def test_gross_margin_review_only_when_sector_wide():
    assert (
        _severity(ReasonCode.GATE_GROSS_MARGIN, market_cap_eur=None, sector_wide=True)
        == SeverityBucket.REVIEW
    )
    assert (
        _severity(ReasonCode.GATE_GROSS_MARGIN, market_cap_eur=None, sector_wide=False)
        == SeverityBucket.BENIGN
    )
...
def test_always_benign_codes():
    for rc in (
        ReasonCode.GATE_MARKET_CAP,
        ReasonCode.GATE_GOING_CONCERN,
        ReasonCode.GATE_ENFORCEMENT,
        ReasonCode.GATE_RESTATEMENT,
...
def test_reconciliation_invariant_full_run():
    skipped = _resolved(
        "SKIP", edgar_skipped="no_cik", dims={"growth": 4, "profitability": 4}
    )
    ...
    basis = BasisFilterResult(
        passed=passed, unresolved=["DEGR", "GONE"], resolved=resolved, degraded=["DEGR"]
    )
    summary, dropouts = build_funnel(
...
def test_relative_rescues_surfaced_on_summary():
    ...
    basis = BasisFilterResult(
        passed=[abs_pass, rescue, untagged],
        unresolved=[],
        resolved=[abs_pass, rescue, untagged],
        degraded=[],
    )
    summary, _ = build_funnel(
        universe=["ABS", "RES", "UNT"],
        basis=basis,
        scored=None,
        score_threshold=4.0,
        crosshits_min_dimensions=2,
...
def test_dry_run_omits_scoring_stages():
    ...
    ok = _resolved("OK")
    basis = BasisFilterResult(
        passed=[ok], unresolved=[], resolved=[vol, ok], degraded=[]
    )
    summary, dropouts = build_funnel(
        universe=["VOL", "OK"],
        basis=basis,
...
def test_sector_wide_excludes_margin_free_sectors():
    fin = [
        _resolved(f"F{i}", sector="Financial Services", basis_reason="gross_margin")
        for i in range(6)
    ]
    basis = BasisFilterResult(passed=[], unresolved=[], resolved=fin, degraded=[])
    summary, dropouts = build_funnel(
        universe=[r.ticker for r in fin],
        basis=basis,
        scored=None,
        score_threshold=4.0,
        crosshits_min_dimensions=2,
...
def test_sector_wide_fires_for_normal_sector():
    ...
    summary, dropouts = build_funnel(
        universe=[r.ticker for r in ind],
        basis=basis,
        scored=None,
        score_threshold=4.0,
        crosshits_min_dimensions=2,
...
def test_sector_wide_denominator_excludes_pre_margin_drops():
    recs = [
        _resolved(f"V{i}", sector="Industrials", basis_reason="avg_volume")
        for i in range(4)
    ]
...
def test_statement_unavailable_maps_to_resolution_statement_unavailable():
    from app.screener.funnel import _BASIS_REASON, ReasonCode

    assert (
        _BASIS_REASON["statement_unavailable"]
        is ReasonCode.RESOLUTION_STATEMENT_UNAVAILABLE
    )


def test_statement_unavailable_dropout_is_review_and_reconciles():
    """RESOLUTION_STATEMENT_UNAVAILABLE is REVIEW (transient) and reconciliation holds."""
    unavail = _resolved(
        "FETCH_ERR", sector="Financial Services", basis_reason="statement_unavailable"
    )
    hit = _resolved("HIT", dims={"growth": 4, "profitability": 4})
    basis = BasisFilterResult(
        passed=[hit],
    ...
    assert ReasonCode.RESOLUTION_STATEMENT_UNAVAILABLE in codes
