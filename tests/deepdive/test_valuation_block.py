...
def test_full_block_with_all_inputs():
    out = render_valuation_block(
        _qs(
            trailing_pe=30.0,
            forward_pe=25.0,
            enterprise_value=2.1e10,
            ebit=2.0e9,
            free_cashflow=1.5e9,
            market_cap=3.0e10,
            total_debt=2.0e9,
...
def test_derived_na_reasons():
    out = render_valuation_block(_qs(trailing_pe=10.0, ebit=1.0e9, free_cashflow=1.0e9))
    # EV missing
...
def test_consensus_line_happy_path_with_implied_upside():
    out = render_valuation_block(
        _qs(
            ...
            target_median_price=110.0,
            number_of_analyst_opinions=42,
        )
    )
    assert (
        "Analyst Consensus: buy · Target: 111.00 (Median 110.0) · "
        "42 Analysten · Upside 11.0%"
    ) in out
...
def test_consensus_upside_na_when_price_zero():
    out = render_valuation_block(
        _qs(price=0.0, recommendation_key="hold", target_mean_price=111.0)
    )
    assert "Upside n/a (Kurs fehlt)" in out
...
def test_consensus_fallbacks_for_missing_optional_subfields():
    ...
    assert (
        "Analyst Consensus: n/a · Target: 120.00 (Median n/a) · "
        "n/a Analysten · Upside 20.0%"
    ) in out


def test_forward_line_happy_path_generic_period_labels():
    fe = ForwardEstimates(
        revenue_growth_cy=0.1485,
        revenue_growth_ny=0.0809,
        eps_growth_cy=0.1714,
        eps_growth_ny=0.1023,
    )
    out = render_valuation_block(_qs(forward=fe))
    # _fmt_pct == f"{v:.1%}"; 0.1485 -> "14.8%" (float repr rounding)
    assert (
        "Forward-Konsens: Revenue 14.8% (lfd. GJ), 8.1% (Folge-GJ) · "
        "EPS 17.1% (lfd. GJ)"
    ) in out
...
def test_forward_line_partial_fields_render_per_field_na():
    ...
    out = render_valuation_block(_qs(forward=fe))
    assert (
        "Forward-Konsens: Revenue 10.0% (lfd. GJ), n/a (Folge-GJ) · "
        "EPS n/a (lfd. GJ)"
    ) in out
...
def test_stage2a_lines_still_present_regression_guard():
    ...
    kap_idx = next(i for i, l in enumerate(lines) if l.startswith("Kapitalstruktur:"))
    cons_idx = next(
        i for i, l in enumerate(lines) if l.startswith("Analyst Consensus:")
    )
    fwd_idx = next(i for i, l in enumerate(lines) if l.startswith("Forward-Konsens:"))
...
def test_peer_block_appended_after_forward_line():
    ...
    lines = out.splitlines()
    fwd_idx = next(i for i, l in enumerate(lines) if l.startswith("Forward-Konsens:"))
    peer_idx = next(i for i, l in enumerate(lines) if l.startswith("Peer-Vergleich"))
...
def _vh_complete():
    return ValuationHistory(
        ...
        ev_ebit=MultipleStats(
            median=18.0, p25=13.1, n_obs=164, span_years=3.1, status="complete"
        ),
        fcf_yield=MultipleStats(
            median=0.038, p25=0.055, n_obs=164, span_years=3.1, status="complete"
        ),
    )
...
def test_valuation_range_line_complete_all_three():
    ...
    line = next(l for l in out.splitlines() if l.startswith("Bewertungs-Range"))
...
def test_valuation_range_prefix_shows_real_span_and_obs():
    out = render_valuation_block(
        _qs(trailing_pe=10.9, valuation_history=_vh_complete())
    )
    line = next(l for l in out.splitlines() if l.startswith("Bewertungs-Range"))
...
def test_valuation_range_line_between_bewertung_and_kapital():
    ...
    rng = next(i for i, l in enumerate(lines) if l.startswith("Bewertungs-Range"))
    kap = next(i for i, l in enumerate(lines) if l.startswith("Kapitalstruktur:"))
    assert bew < rng < kap
...
def test_valuation_range_ttm_leg_matches_bewertung_line():
    ...
    rng = next(l for l in out.splitlines() if l.startswith("Bewertungs-Range"))
