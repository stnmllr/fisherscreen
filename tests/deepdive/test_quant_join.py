...
def test_pit_from_cache_when_present():
    fs, yf, hist = _deps(
        pit_cache={"shortName": "Novo Cached", "marketCap": 2.9e11, "currency": "DKK"},
...
def test_currency_note_when_financial_currency_differs():
    fs, yf, hist = _deps(pit_cache={"currency": "USD", "marketCap": 1}, dims=None)
    qs, cov = build_quant_snapshot(
        "X",
        firestore=fs,
        yfinance=yf,
...
def test_partial_historical_flagged():
    ...
    hist.get_annual_series.return_value = {
        ...
        "years": [2024, 2023],
        "revenue": [2, 1],
        "gross_margin": [0.8, 0.8],
        "operating_margin": [0.4, 0.4],
        "shares_outstanding": [9, 9],
        "buyback_cashflow": [0, 0],
        "complete": False,
    }
...
def test_resolve_dividend_yield_msft_class_glitch_corrected():
    ...
    assert _resolve_dividend_yield(0.81, 0.25, 28.0) == pytest.approx(0.0081, abs=1e-5)
...
def test_resolve_dividend_yield_ko_no_double_correction():
    ...
    assert _resolve_dividend_yield(2.68, 0.70, 26.0) == pytest.approx(0.0268, abs=1e-4)


def test_resolve_dividend_yield_legit_fraction_regime_untouched():
    # old fraction regime: implied 0.025, ratio < 10 -> kept untouched
    assert _resolve_dividend_yield(0.024, 0.30, 12.0) == pytest.approx(0.024, abs=1e-4)
...
def test_forward_estimates_wired_onto_snapshot():
    fs, yf, hist = _deps(pit_cache={"marketCap": 1}, dims=None)
    fe = ForwardEstimates(revenue_growth_cy=0.1485, eps_growth_cy=0.1714)
