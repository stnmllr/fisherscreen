...
def test_coverage_label_ok_partial_empty_failed():
    ok = insider_coverage_label(
        InsiderSummary(
            coverage_state="ok",
            n_filings_total=10,
            n_parsed=10,
            significant_buys=[
                InsiderTransaction(owner_name="A", role="CEO", code="P", bucket="buy")
            ],
        )
    )
...
def test_parse_form4_empty_document_returns_empty_list():
    assert (
        parse_form4("<?xml version='1.0'?><ownershipDocument></ownershipDocument>")
        == []
    )
...
def test_schema_version_mismatch_is_cache_miss(tmp_path):
    ...
    p.parent.mkdir(parents=True)
    p.write_text(
        json.dumps({"schema_version": 999, "transactions": []}), encoding="utf-8"
    )
    edgar = _Edgar([ref], _XML)
    res = CachedInsiderFetcher(edgar=edgar, cache_dir=tmp_path).get_summary_input(
        "1", "2025-01-01"
