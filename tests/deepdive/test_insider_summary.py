...
def test_three_bucket_netting_and_reconciliation():
    ...
    assert s.n_transactions_total == (
        len(s.significant_buys)
        + len(s.significant_sells)
        + s.immaterial_sell_count
        + s.routine_count
    )
...
def test_ceo_sell_significant_regardless_of_value():
    ...
    s = compute_insider_summary(
        txns, coverage_state="ok", n_filings_total=1, n_parsed=1
    )
...
def test_per_owner_aggregate_marks_all_constituents():
    ...
    s = compute_insider_summary(
        txns, coverage_state="ok", n_filings_total=3, n_parsed=3
    )
...
def test_by_role_aggregation():
    ...
    s = compute_insider_summary(
        txns, coverage_state="ok", n_filings_total=2, n_parsed=2
    )
