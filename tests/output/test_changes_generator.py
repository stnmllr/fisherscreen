...
def _write_prior_dimensions(
    output_dir: Path, month: str, tickers_per_dim: dict[str, list[str]]
) -> Path:
...
def test_new_ticker_in_dimension_is_highlighted(tmp_path):
    ...
    path = generate(
        records, _run_record("2026-05-13T08:00:00+00:00"), tmp_path, score_threshold=4.0
    )
...
def test_gap_in_history_uses_most_recent_prior_file(tmp_path):
    ...
    path = generate(
        [_record("Y", growth=4)],
        _run_record("2026-05-13T08:00:00+00:00"),
        tmp_path,
        score_threshold=4.0,
    )
    content = path.read_text(encoding="utf-8")
...
def test_corrupt_frontmatter_falls_back_to_no_prior(tmp_path):
    ...
    universum.mkdir()
    (universum / "2026-04-Dimensions.md").write_text(
