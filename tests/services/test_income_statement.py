...
def _make_stmt(rows: dict) -> pd.DataFrame:
    ...
    return (
        pd.DataFrame(rows, index=list(rows.keys()))
        if False
        else pd.DataFrame(
...
def test_none_value_in_row_returns_none():
    stmt = _stmt(
        **{"Total Revenue": None, "Cost Of Revenue": 300.0, "Gross Profit": 700.0}
    )
    available, rev, cor, gp, cor_present = extract_waterfall_inputs(stmt)
    assert available is True
    assert rev is None
...
def test_extract_revenue_series_oldest_to_newest():
    stmt = _multi_year_stmt(
        {"2024": 130.0, "2023": 120.0, "2022": 110.0, "2021": 100.0}
    )
...
def test_extract_revenue_series_drops_nan_and_nonpositive():
    stmt = _multi_year_stmt(
        {"2024": 130.0, "2023": float("nan"), "2022": -5.0, "2021": 100.0}
    )
