...
> 1.389 Tickern — tatsächlich sind es **1709 Tests / 96,26 %** und **1.322 Ticker**. Auch
...
**Die Stetigkeits-Dimension ist gebaut, gemergt und deployt** (PR #62, `main` `f76b01a`;
davor #60 Preisnehmer-Kennzeichnung, #61 Spec-Entscheidungen). Reihenfolge nach Spec §9:
...
**Im Code, aber noch nicht in Wirkung:** die Achse greift erst beim nächsten Monatslauf.
`steadiness_stale` und `steadiness_missing` sind verdrahtet und getestet, haben aber noch
nie in `dev_screener_runs` gestanden. `crosshits_min_dimensions` bleibt bei 3.
...
**Die CI hat einen Fehler gefangen, den `lokal grün` strukturell nicht finden konnte:**
`app/main.py` baut jetzt den Jahresreihen-Cache, der einen leeren EDGAR-User-Agent
ablehnt — lokal liefert `.env` einen, die CI hat keins. Drei `test_main`-Tests mockten den
neuen Builder nicht. Test-seitig behoben, kein Produktivcode geändert. **Neue Routine vor
jedem Push, der die Komposition anfasst:** Suite mit leer gesetzten Credential-Variablen
laufen lassen — Env sticht `.env`, das reproduziert die CI-Umgebung.
...
Der Bau ist freigegeben, kommt aber als eigener PR (→ Block 2026-09-08).
...
2 Dateien übersprungen, exit 0**; volle Suite 1312 passed, 97,73 %.
Commits: `8b37e33` (CLAUDE.md-Revision + Join-Key-Ticket) → `025016f` (Parser +
Modelle) → `782f76e` (Defekt-Tabelle, Escaping, CSS, Übersicht) → `367ea9b` (Detailseite,
Site-Generator, CLI); `3dca6a4` verschärft das Ticket.

Zwei Entwurfsentscheidungen weichen bewusst von der Spec ab: die geforderte
`SUPPRESSED_METRICS`-Sperrliste wurde durch eine **datierte Defekt-Tabelle**
(`app/viewer/defects.py`) ersetzt — Werte bleiben sichtbar und bekommen ein ⚠, statt
ausgeblendet zu werden. Auslöser war ein **vierter** Fehler im Altbestand, den eine
handgepflegte Liste nicht enthalten hätte: der Div-Yield-Prozent-Glitch in 9 Dossiers
(GOOGL 23,0 %, MSFT 86,0 %, ASML 59,0 %, real je unter 1 %), gefixt erst am 2026-06-01.
Und Phase 1 enthält **keinen Deploy-Code**.

**OFFEN:** Merge-Freigabe · Zielsystem ist Caddy + Google-OAuth auf `macro.stnmllr.com`
(nicht nginx/Basic-Auth wie in der Spec angenommen; ein Deploy-Skript existiert dort nicht)
· Entscheidung zu `markdown-it-py` und `black` als deklarierte Abhängigkeiten
· separater Datenschicht-PR für EV-Guard und D/E-Einheit in `quant_join.py`.

---

**Tier-B Punkt 2 — FERTIG, LIVE IN PROD & PROD-SMOKE-VERIFIZIERT 2026-06-10 (PR #30 → main, Deploy `27278108537` success).** Relativer gross_margin-Arm aktiv (k=0,5, v4-Table 52 Buckets). **Kalter Prod-Smoke (Gate-B-in-Prod) GRÜN:** `relative_rescues: n=112` (>0, NICHT dormant — der Arm feuert live), Health grün (`unresolved=5` exakt UNCLEAR-Satz, `going_concern_drops=0`, `edgar_filter 723/731`, `data_source_error=3` transient). **n=112 vs Gate-B-Lokal 175 erklärt+rigoros verifiziert:** 175 isolierte das gross_margin-Gate; die 112 sind Relativ-Rettungen, die den VOLLEN Basis-Filter überleben — der nachgelagerte revenue_growth-Gate (Punkt 3, ≥0) droppt 61 niedrig-margige Zykliker mit negativem TTM-Wachstum (Autos/Homebuilder/Chemie/Grocery), 175→114 lokal ≈ 112 prod; kein Bug, Arm rein additiv. Provenance eingefroren (`0552582`, blobs e0f884c7+293ea92d), Rationale `calibration.md` (PHASE E FINAL `5330503`). Rückweg trivial (`GROSS_MARGIN_RELATIVE_K=None`). **NEXT = Punkt 3 (revenue_growth gescort/geglättet), eigener Zyklus.** Verlauf ↓. Kette: CT-A (d7d4faf) → SEAM-2 fail-loud (e7aee81) → A1-Update+SEAM-4 (0d197a7) → CT-B-Mechanism (75da1d6) → Akzeptanz-Härtung+Cleaning (a5e7aa9) → **v4 absoluter Below-Median-Regime-Gap-Detektor (750f5c4)** → **Phase E (76b6504): v4-Table (52 Buckets, vintage 2026-06) + `GROSS_MARGIN_RELATIVE_K=0.5`**. **Gate-B Cold-Run ($0) GRÜN:** Additivität ok (Pass(0,5) ⊇ Pass(None), 884→1059, kein Vor-Passer fällt), Identity ok (RELATIVE_RESCUE-Tag == |DELTA| == 175), Vorhersage-Match (175 == A3-k=0,5). k=0,5 = maximal-clean Decke (erster Gesunder AES kippt bei k>0,514); harte akzeptierte False-Negatives (Pre-Scoring-Knock-outs, kein k rettet sie ohne Floor-Masking): Costco/Accor/Tesco/Sainsbury. ReasonCode `ABSOLUTE_PASS`/`RELATIVE_RESCUE` (Sub-Floor-Def) als Prod-Audit. 6 Map-Flags geprüft: 5 irrelevant (Groups nicht pinned), Rental&Leasing→Capital-Goods behalten (load-bearing aber Akzeptanz-sauber, spread 0,087; Umremappen wäre Margen-Gerrymandering der exogenen Achse). Detail ↓. Die Maschine wurde über Nacht mitten in der CT-A-Aufgabe neu gestartet; Zustands-Rekonstruktion aus dem Repo (nicht aus Annahmen) ergab: **nichts ging verloren** — die CT-A-Runtime-Verdrahtung wurde am 2026-06-09 23:39 unbeaufsichtigt committet (`d7d4faf`), Tree clean, kein Stash/Orphan, Suite grün (858 passed, 97,10 %). **Settled Design-Entscheidungen (im Chat gelockt, standen vorher in KEINER Datei — hier festgehalten, damit sie nicht erneut verloren gehen):** (1) **3-Zustands-Feld statt bool-Flag** — `ScreenerRecord.definedness: DefinednessOutcome | None` (METRIK_NA / UNASSESSABLE / DEFINED; None = nicht assessed) trägt den Fetch-Fehler getrennt vom strukturellen UNDEFINED; ein bool `metric_na` würde beide wieder kollabieren — exakt die `[[distinguish-failure-from-empty-result]]`-Falle, gegen die der 44b1135-Refactor gebaut wurde. Das ist eine **Korrektur**, keine Abweichung; Rückdrehen auf bool wäre Regression. (2) **ReasonCode = `RESOLUTION_STATEMENT_UNAVAILABLE`** (umbenannt von `FRAMEWORK_STATEMENT_UNAVAILABLE`, Commit `500205a`, verhaltensneutral, grün): das Präfix folgt der **Semantik** (nötige Daten-Holung schlug fehl → divert → retry — Geschwister von `RESOLUTION_FX_UNAVAILABLE`/`RESOLUTION_NO_SYMBOL_DATA`), nicht der Stage; bewusst NICHT mit dem permanenten, strukturellen `FRAMEWORK_METRIK_NA` gruppiert. Empirisch abgesichert: **kein Rollup/View keyt auf Präfix** (CSV/JSON emittieren volle `reason_code.value`; `_ALWAYS_REVIEW`/`_severity` sind explizite Member-Sets; Stage-Counts = Mengen-Arithmetik) → niedrige Stakes, beide divertet via `_ALWAYS_REVIEW`, Reconciliation schließt. **Platzierung bleibt:** Divert an der BASIS-Stage an der metric_na-Position (vor `gross_margin`), transient/retrybar, weder passed noch failed. (3) **Runner-Pre-Pass** (`_assess_definedness_basket`): Ordnung resolve → Volume+Cap → (NUR Korb ∩ Vol+Cap-Survivors) income_stmt-Fetch + `assess_definedness` → Flags → Rest Basis-Filter; REIT-Kurzschluss (Industry-Substring) vor Fetch; Fetch im Runner, Filter rein (lesen nur `record.definedness`); Korb-Scoping ist FETCH-Scoping mit Rekalibrierungs-Trigger „Positiv-Kante ⊆ Financials/RE beim Re-Vintage prüfen". (4) **Shared Extractor** `extract_waterfall_inputs` (`app/services/income_statement.py`) — wird als Nächstes von A1 wiederverwendet. **Rettungs-Arm bleibt DORMANT:** `GROSS_MARGIN_RELATIVE_K = None`, kein committeter Sektor-Median-Table. **STATUS = Runtime VERIFIZIERT** durch ein **unabhängiges adversariales Review-Gate** (Subagent, nicht Selbstbenotung — grüne Suite + Selbst-✓-Tabelle wäre genau die Selbstgrading-Falle; d7d4faf war die eine substanzielle Unit, die das Review-Gate über Nacht übersprang). Vier Seams geprüft, Befunde unabhängig nachverifiziert: **SEAM 1 (Fetch-Fehler-Pfad, die Kern-Sorge) ✅** — Fetch-Fehler → `DataSourceError` (yfinance_client.py:77-85 wrappt *jeden* Fehler, gibt *immer* ein Literal-3-Tupel → `[0]` nie IndexError) → vom Runner gefangen → `assess_definedness(statement_available=False)` → **UNASSESSABLE**, nie Kollaps zu METRIK_NA, getestet; **SEAM 3 (Reconciliation) ✅** — der neue `RESOLUTION_STATEMENT_UNAVAILABLE`-Eimer wird real geübt, Reconciliation schließt mit ihm present, kein Doppel-Count/Drop; **SEAM 2 (Survivor-Gating) — DEFEKT gefunden + gefixt (`e7aee81`):** ein `except Exception: continue` um die Vol/Cap-Gates schluckte Throws **still** → `definedness=None` (non-assessed) → ein spurios-positiver Financial/REIT, der METRIK_NA sein müsste, rutscht als **DEFINED-by-default** zum gross_margin-Gate und ins Scoring (die via-Positiv-/BNP-/ACA-Sorge, wieder offen, unlogged, 0 % Coverage). Fix = **fail-loud**: Catch-all raus, jede Filter-Exception propagiert; der explizite `DataSourceError→UNASSESSABLE`-Pfad bleibt. +3 TDD-Tests (Propagation `FilterConfigError` + arbiträre Exception, Regression `→UNASSESSABLE`), 861 grün / 97,17 %; **SEAM 4 (Shared Extractor) — latente Fragilität, bewusst zur A1-Unit GETRAGEN:** `extract_waterfall_inputs` ist nicht robust gegen einen pandas-**Series**-Input (`AttributeError` statt graceful `(False,…)`); aus dem Runtime **unerreichbar** (immer DataFrame/None) → prinzipientreu erst dort gehärtet, wo A1 die zweite Call-Site real macht (Guard gegen A1s *echten* Input-Shape, nicht spekulativ jetzt — sonst toter Defensiv-Code, der das Falsche fängt). **A1-UPDATE-UNIT ✅ ERLEDIGT 2026-06-10 (`0d197a7`):** (a) **SEAM-4 geschlossen** — `extract_waterfall_inputs` degradiert nicht-2-D-Input (Series/Scalar) zu not-available statt `AttributeError`; **Befund beim Bau:** A1 füttert real einen DataFrame (wie der Runner) → der Guard ist eine **Contract-Assertion** (`ndim==2`), kein Series-spezifischer Patch (keine Call-Site erzeugt eine Series → kein toter Defensiv-Code); +2 TDD-Tests; (b) **REIT-Check in A1 gezogen** — A1s METRIK_NA-Set wird jetzt aus `assess_definedness` (Runtime-Single-Source: REIT-Kurzschluss + Wasserfall) gebaut, spiegelt exakt den Runner-Divert inkl. REITs (verdict=DEFINED aber outcome=METRIK_NA = Property-C-Evidenz); (c) A1 konsumiert den Shared Extractor (private `_first_col_value`-Kopie entfernt). UNASSESSABLE (Fetch-Failure) separat getrackt, **nie** im METRIK_NA-Set. 863 grün/97,17 %. Rettungs-Arm bleibt DORMANT (`GROSS_MARGIN_RELATIVE_K=None`, kein committeter Table). **CT-B-MECHANISM + AKZEPTANZ-INSTRUMENT ✅ GEBAUT 2026-06-10 (`75da1d6`, 895 grün):** `_node_chain` rollt industry→GICS-Industry-Group (margenblinde **exogene** Achse) statt multimodalem Sektor; thin-ohne-Group-Mapping → `[industry]` → kein Bucket → Fail-Safe (Sektor nie konsultiert); propagiert über den einen `_node_chain`-Punkt auf Prod-Gate + A2 + A3. Map `data/gics_industry_group_map.json` (exogen, first-cut, gegründet im echten thin-Industry-Histogramm via `scripts/diagnose_industry_histogram.py`; **6 Cross-Taxonomie-Ambiguitäten in `_meta` GEFLAGGT — brauchen Stephans Taxonomie-Call**: Education&Training, Pharma-Retailers, Pollution&Treatment, Rental&Leasing, Solar, Recreational Vehicles; das Dispersions-Gate fängt multimodal-gewordene Buckets, NICHT eine spurious-kohärente Fehlzuordnung). Validierender Loader (absent→`{}` fail-safe, malformed→`FilterConfigError`). Instrument `scripts/diagnose_bucket_dispersion.py` = **unabhängiges Akzeptanz-Gate** (per Bucket: median/IQR/std/MAD, Konstituenten-Median-Spread=Multimodal-Detektor >0,15, Bimodalitäts-Koeffizient >0,555 + ASCII-Histogramm, **clean-rescue-set-Headline**). **GROUNDING-FINDING (Probe VOR dem Bau, $0): saubere GICS-Group-Unimodalität trägt empirisch NICHT für die Schlüssel-Thins** (Railroads 0,46 vs Trucking/Marine 0,24 in Transportation; Consulting 0,42 vs Staffing 0,11 in Comm&Prof-Services; Basic-Materials-Group==Sektor==multimodal) → **Maersk runtergestuft** (war Annahme, kein Befund; verfehlte Rettung ≠ Fehl-Reject, Fehl-Inclusion ist der gefährliche Fehler). **VORAB-FESTGELEGTE VERDIKT-REGEL:** clean-rescue-set nicht-trivial (mehrere homogene Low-Margin-Groups n≥8, Gate hält) → relativer Arm gerechtfertigt, ship (Maersk-out = akzeptierte Scope-Grenze); quasi-leer → relativer Arm **vestigial** → Punkt 2 nur mit Definedness-Exclusion + absolutem Floor shippen, relativen Arm streichen (ehrlich, einfacher, kein Misserfolg). Hand-getrimmte Margen-Gruppen methodisch RAUS (= Gummistempel-Gate, [[adaptive-stat-swallows-judgment]]); Ausstiegsluke nur **andere margenblinde Taxonomie**, nie Trim-nach-Outcome. Relativer Arm bleibt DORMANT (`GROSS_MARGIN_RELATIVE_K=None`, kein committeter Table). **AKZEPTANZ-HÄRTUNG + CLEANING-VEREINHEITLICHUNG ✅ (`a5e7aa9`, 918 grün):** Stephans Dispersions-First-Look (auf frischem A1, 220 METRIK_NA inkl. REITs) ergab clean-rescue-set **78/305** — getragen vom homogenen Low-Margin-Kern (Grocery 0,21 / Integrated Freight 0,18 / Discount 0,28 / Packaged Foods 0,32) → **Arm gerechtfertigt, nicht vestigial**; Maersk löst korrekt auf Fail-Safe (Marine Shipping → Transportation multimodal, constituent-spread 0,34). Zwei Impl-Lücken geschlossen vor k: **(i) Single-Industry-Multimodalität rutschte durch** (BC unterdetektiert — SBS/Travel als „clean") → **bin-freier Antimode-Detektor** (größter interner Gap ≥4× typische Nachbar-Distanz, ≥20% Masse je Seite) ersetzt BC-als-Reject (BC nur noch advisory); validiert gegen SBS/Travel→reject ∧ Specialty-Chemicals(breit-unimodal)/Grocery(tight)→pass; das Gate gatet jetzt das **Pinnen in A2** (rejected Bucket → nicht gepinnt → nicht in counts → `resolve_bucket`=None → Runtime-Fail-Safe by construction). **(ii) Cleaning divergierte** (A2 Negative-rein/1097 vs Probe Negative-raus/1030 → Median-Drift, Analyse gegen nicht-deploybare Mediane) → **eine geteilte `clean_universe`** (METRIK_NA+Negative raus, KEIN Sektor-String → Capital-Markets-DEFINED bleiben) für A2/A3/Instrument. Die 78 waren dadurch inflationiert (SBS/Travel-Phantome) → schrumpfen beim Re-Run Richtung Kern, Arm bleibt gerechtfertigt. **NEXT = Stephan: (0) die 6 Map-Ambiguitäten prüfen/überschreiben, (1) A1→A2→A3 + `diagnose_bucket_dispersion.py` neu fahren auf der gehärteten/vereinheitlichten Pipeline → clean-rescue-set neu (justified vs vestigial); falls justified → k gegen sauberen Table benennen → Phase E** (Table+k committen, Gate-B Cold-Run, gated). **Kein Push/Merge ohne Stephans Go.** Spec/Plan `docs/superpowers/specs/2026-06-09-punkt-2-sector-aware-gross-margin-floor-design.md`; Memory [[punkt2-sector-gross-margin-floor-state]], [[distinguish-failure-from-empty-result]], [[adaptive-stat-swallows-judgment]].

**Gebündelter Tier-A-PR #25 {Funnel-Instrumentierung + 0a + 0b + Punkt 1} GEMERGT, DEPLOYT & im PROD-SMOKE-TEST BIT-GENAU VERIFIZIERT 2026-06-09 (nach einem vom Smoke-Test gefangenen 500er + PR-#27-Fix) — und eine falsche „bereits deployt"-Annahme korrigiert.** Eine Vorsession hatte notiert, dieses Bündel sei am 06-08 „via PR #25 → main → Cloud-Run-Prod" bereits ausgeliefert. **Das war falsch — git-verifiziert:** der Branch *war* gepusht und **PR #25 war offen** (CI `test=pass`, `MERGEABLE/CLEAN`), aber der **Merge-Klick fehlte** → `merge-base --is-ancestor` zeigte den Gate-Tip NICHT in `main`, `origin/main` stand auf `67ba77e` (PR #24, 06-05), Prod fuhr also weiter Stand PR #24. Stolperfalle bei der Diagnose: lokal war der Branch-Ref gelöscht (Commits nur reflog-erreichbar) → sah kurz nach „kein PR #25" aus, bis der Push-Versuch (non-fast-forward) + `gh pr list` den offenen Remote-PR offenlegten. **Lehre:** lokaler orphaned-Branch ≠ „nie existiert" — immer gegen `origin` + `gh pr list` prüfen, bevor man „kein PR" schließt ([[plan-doc-verify-against-code]]). **Heute ausgeführt (mit Stephans Go):** Pre-Merge-Gate lokal grün (`uv run python -m pytest -m "not integration"` → **794 passed, 8 deselected, 97,04 %**); PR #25 gemergt (Merge-Commit `25e8eae`, `mergedAt 07:11:29Z`); Push/Merge liefen über Stephan, da die Session-Sandbox `git push`/`gh pr merge` blockt. **Deploy verifiziert:** `deploy.yml`-Run **`27190036940` `conclusion=success`** (2m8s), beide kritischen Steps grün („Build and push image" + „Deploy to Cloud Run" — letzterer meldet nur Erfolg, wenn die neue Revision Ready/serving wird); `origin/main` `67ba77e..25e8eae`, lokaler `main` ff'd. **Aber Deploy-grün ist NICHT Verifikation:** der erste Prod-Smoke-Test (`POST /run/monthly?dry_run=true`, $0) crashte mit **HTTP 500 / `ModuleNotFoundError: app.output`** — ein Build-Context-Bug aus dem Hygiene-Cleanup (ein **unanchored `output/`** in `.gitignore` warf das Source-Package `app/output/` aus dem gcloud-Build-Context), den der grüne Deploy **maskierte** und erst der Smoke-Test fing. Fix via **PR #27 (`8757bb3`)** = `/output/` root-anchored, siehe [[gcloud-build-context-gitignore-derived]]. **Re-Smoke-Test nach Redeploy = GRÜN, bit-genau zur lokalen GATE-B-Prognose:** `edgar_filter 698/706` (Survivor **698**, +10 exakt), `basis_filter 706/1314` (608 basis-fail), 8 Restatement-Drops, `unresolved=5` exakt {AMS.VI, RIGN.SW, ROL.L, SANO.HE, SCHA.OL}, `going_concern_drops=0`, `funnel_summary.json` geschrieben (624 dropouts, 101 review-flags), `data_source_error=1` (DKS transient); Reconciliation **`1322 = 698 + 608 + 8 + 8`** schließt. Caches vorab 2× cold-purged (1317 ticker + 552 edgar), dann vom neuen Code befüllt. **Lehre (verstärkt): Deploy-grün ≠ verifiziert — der echte Prod-Smoke-Test ist der Schiedsrichter, genau er fing hier den Bug** ([[prod-logging-dormant-and-cache-masks-verification]]). Damit sind **Funnel-Instrumentierung Phase 1, 0a (RIC-/FR-Kontaminanten, 1332→1322), 0b (Resolution-Daten-Qualität) und Punkt 1 (wert-basiertes Volumen-Gate)** zusammen live. **NEXT = Tier B** (Punkt 2 gross_margin sektor-relativ + Financials out-of-scope / Punkt 3 revenue_growth gescort/geglättet) als eigener Spec + Go; kanonischer Input (jetzt in `main` deterministisch reproduzierbar): die 13 doomed-rescued + Roche + die Financials-Auslöschung. Detail-Verlauf: Memory [[gate-universe-fixes-progress]]. Vorgeschichte ↓.

**Wert-basiertes Volumen-Gate (Ticket Punkt 1) — Code FERTIG + reviewt, GATE-A €1M ABGENOMMEN, GATE-B Cold-Run GRÜN (+10 Survivors, bitgenau prognostiziert) (Branch `feature/gate-universe-fixes`, UNPUSHED) — 2026-06-08.** Erster **survivor-ändernder** Fix (Acceptance: „das Richtige hat sich geändert"). Stück-Floor (`avg_daily_volume≥100k`) → **Ø-Tages-Handelswert in EUR** (`vol×price×fx_rate≥Schwelle`); hochpreisige wenig-Stück-aber-liquide Titel (Lindt LISN €18M/Tag, 175 Stück) waren falsch ausgeschlossen. **Vier Bausteine (TDD, sub-agent-driven, zweistufig reviewt):** (A) **Pence/Minor-Unit-Normalisierung** bei Konstruktion — Londoner `.L` quotieren in Pence (`currency='GBp'`, price in Pence, marketCap aber GBP) → `vol×price×fx` war **100× zu hoch** (GAW.L €2,05bn statt €20,5M); `_MINOR_UNIT={"GBp":("GBP",100)}` ÷100 + currency→ISO an der Datengrenze (market_cap_eur unberührt, regressions-getestet); price `or None` (0→None). (B) **fx_rate-Primitiv** am Record (carry aus Resolution, statt downstream re-derive). (C) **NO_PRICE-Resolution-Divert** (0b-Geschwister — price wird load-bearing → fehlend/0 = REVIEW, nicht still am Gate; Präzedenz mc→cur→vol→price→fx). (D) **Wert-Gate mit Fail-Loud-Sentinel** `MIN_AVG_DAILY_VALUE_EUR=None` → raised „not calibrated" in echtem Lauf (unkalibriert nicht shippbar) + raising-Guard bei fehlendem Input (Invarianten-Verletzung, kein stiller Drop); Severity keyt auf market_cap_eur (FER/1COV bleiben REVIEW). 801 Tests grün, 97,3 %, kein Drift (inkl. Deepdive). **GATE A (Kalibrierung, $0, abgenommen):** Schwelle **€1.000.000/Tag** — Begründung **strukturell, nicht 30-Namen-optimiert**: Histogramm aller 688 Survivors zeigt min=€2,45M, p50=€169,6M, **null unter €2M**; broken/micro enden bei €0,89M → **leeres €0,89M–€2,45M-Band (2,75×-Lücke = Liquiditäts-Schichtgrenze)**; €1M ist absoluter Trading-Minimum-Anker (nicht relativ → drift-robust), 2,45× unter dem nächsten Survivor, droppt **null** aktuelle Survivors, hält FER/1COV (broken-avgVol) korrekt drunter→REVIEW. Survivor-Split-Prognose @€1M = **+10** (688→~698: FLTR,GAW,GHC,LISN,LPP,MELE,SIX2,SLHN,VACN,VCT), 13 fallen an Folge-Gate (Punkt 2/3, nicht vermischt), GATE_VOLUME-Drops 27→4. FX-Pfad NOK/SEK verifiziert (0,0915 EUR/Einheit, nicht invertiert). **Reversibilitäts-Trigger (für Future-Stef — nicht wegrationalisieren):** (1) Anker = die *Lücke*, nicht €1M — re-evaluieren wenn `min(Survivors) < 2× max(Broken/Micro)`; (2) Kanarienvögel namentlich tracken: **LSG.OL · WIHL.ST · SWEC-B.ST** (untersten 3 Survivors €2,45/3,72/4,64M) — fällt einer raus ohne Ersatz = Verteilungsdrift-Signal, nicht Floor-Anpassung; (3) Minor-Unit/FX bei Universen-Erweiterung gegen den Anker verifizieren. Artefakt: `docs/superpowers/audits/2026-06-08-1-value-floor/calibration.md`; Spec/Plan `docs/superpowers/{specs,plans}/2026-06-08-value-based-volume-gate-1*`; Proben `scripts/diagnose_{value_floor_calibration,survivor_value_histogram,p1_survivor_split,canary_fx}.py`. **GATE B (Cold-Run 2026-06-08) GRÜN:** edgar-Survivor **688→698 (+10 bitgenau)**, die 10 prognostizierten Namen alle present, die 13 doomed-rescued alle an gross_margin/rev_growth (Punkt-2/3, nicht vermischt), GATE_VOLUME 27→6 (die 4 + 2 saubere bidirektionale Funde BPOST.BR/ONTEX.BR = Stück-Floor-Passer mit <€1M-Wert, Small-Caps, null Survivor-Effekt), NO_PRICE-Diverts keine, Wert-Gate raised nirgends, Reconciliation `1322=8+608+8+698`. Artefakt: `docs/superpowers/audits/2026-06-08-1-value-floor/gateB_acceptance.md`. **NÄCHSTER SCHRITT: gebündelter Remote-PR {Funnel-Instrumentierung + 0a + 0b + Punkt 1} = EIN Prod-Deploy (Stephans Go — Push auf main triggert Deploy).** Vorgänger 0a/0b siehe Memory [[gate-universe-fixes-progress]].

**Roche illiquide-Linie GEFIXT, gemergt, deployt & im Cold-Run VERIFIZIERT 2026-06-05 — systematischer Dual-Line-Sweep = n=1.** Das Universum ankerte `RO.SW`, die illiquide Roche-**Inhaber**-Linie (avgVol 36k < 100k-Volumen-Gate → konnte die Basis nie passieren). Swap auf **`ROP.SW`** = liquider Roche-**Partizipationsschein**/Genussschein (yfinance-shortName „ROCHE PS", avgVol 1,02 Mio, SIX). **Korrigiert eine frühere FALSCHE Schlussfolgerung** (06-04-Eintrag: „liquide ROG.SW fehlt") — `ROG.SW` ist tot (404), aber `ROP.SW` löst auf; mein manuelles Probing übersah es, der systematische OpenFIGI-Sweep fand es. **Sweep (n=1):** über alle 27 Universe-Ticker, die auflösen aber am Volumen-Gate sterben (bitgenau gegen die Pipeline-`avg_volume`-Dropmenge 51 = 27 numerisch-illiquide + 24 None-Volume), ist Roche der **einzige** fixbare Bucket-A; jeder andere illiquide Name ist Single-Class oder hat nur illiquide Schwestern (Share-Count-Artefakte hochpreisiger Aktien, z.B. Lindt LISN/LISP beide <100k). Dual-Class-anfällige Namen hand-verifiziert (Sixt Stamm/Vorzug, Maersk A/B, Lindt PS) → kein weiterer A. **Methode (wiederverwendbar für die 5 UNCLEAR):** OpenFIGI `/mapping` TICKER+exchCode (saubere Share-Class-Identität) + `/search` (Schwester-Enumeration, normalisierter Namens-Match) + yfinance als Ground-Truth (Auflösung+Liquidität). **yfinance `.isin` ist unbrauchbar** (lieferte für RO.SW Roche *Bobois* FR0013344173) — Identität nur aus OpenFIGI. Zwischendurch trat die Masking-Klasse erneut auf (geschluckter 429 → falsch-SINGLE_LINE für alle 27) → gefixt (Backoff + harter Such-Fehler → NEEDS_MANUAL, nie SINGLE_LINE). **Disziplin:** assertion-guarded Transform (`apply_roche_swap.py`, net-0), 739 Tests grün, eigener PR #24 (Merge `67ba77e`), Deploy rev `00087-cjk`, Cold-Run nach Doppel-Cache-Purge GRÜN: `unresolved.count=5` exakt `{AMS.VI,RIGN.SW,ROL.L,SANO.HE,SCHA.OL}` (ROP.SW NICHT dabei → löst auf), `RO.SW` weg, `going_concern_drops=0`, `data_source_error=0`, `no_cik=143` (Live-Drift vs 140@06-04, ROP.SW nicht darunter). **Caveat — der Swap holt Roche NOCH NICHT in den Screen:** `ROP.SW` passiert Volumen/gross_margin/market_cap, dropt aber am `revenue_growth` (TTM −0,4 % < 0) — d.h. Roche ist jetzt **allein vom revenue_growth-Gate** blockiert (Ticket 3), kein Daten-Defekt mehr. Doku: `docs/superpowers/audits/2026-06-05-dual-line-sweep/` (README + reproduzierbare Instrumente) + retracted Roche-Eintrag in `docs/superpowers/known-data-limitations.md`. Memory: [[roche-dual-line-fix-and-sweep-n1]], [[distinguish-failure-from-empty-result]].

**Universum-Vollständigkeits-Audit + stille-Attrition-Bug GEFIXT, DEPLOYT & im Cold-Run VERIFIZIERT 2026-06-04 — degradierte-Dict-Maskierung GESCHLOSSEN.** Unabhängiges, reproduzierbares Audit (`docs/superpowers/audits/2026-06-03-universe-completeness/`, Funnel + eigene Re-Resolution aller 1349 + 1×-Reprobe der Failures) falsifizierte den Cold-Run-Self-Report: **`yfinance_unresolved=0` war irreführend** — 35 Universe-Ticker lösen unabhängig NICHT auf, **0 davon transient**, maskiert weil `get_ticker_info` degradierte 404-`.info`-Dicts (nicht-leer, ohne name/marketCap) als Erfolg wertete → Titel zählte „resolved", fiel still am basis_filter als Missing-Field. Klassifikation der 35: **13 reale Firmen still ausgeschlossen** (falsches Ticker — Ericsson `ERICb.ST→ERIC-B.ST`, Atlas Copco, H&M, ASM Intl `ASMI→ASM.AS`, DSM-Firmenich, Flutter, ICG, Teva, Indivior, Barratt Redrow `BDEV→BTRW.L`, Aker BP, EFG, Investec), **12 echte Delistings**, 5 benigne Dubletten, **5 UNCLEAR** (AMS.VI, RIGN.SW, ROL.L, SANO.HE, SCHA.OL — bewusst nicht geraten). **STUFE A (PR #21, Merge `63e663f`):** `get_ticker_info` wirft jetzt bei „kein name UND kein marketCap" → Attrition laut im Aggregat statt still (TDD Rot unabhängig bewiesen). **STUFE B (PR #22, Merge `af49e4a`):** `universe.json` **1349→1332** (13 In-place-Korrekturen + 5 benigne Löschungen + 12 Delisting-Prunes; 5 UNCLEAR unangetastet; assertion-guarded, 0 unbeabsichtigt), Generator `build_universe.py` konservative Nordic-Klassensuffix-Normalisierung, 17 neue Tests, Suite 739 grün. **Cold-Run-Verifikations-Gate 2026-06-04 GRÜN (das eigentliche DONE):** Pflicht-Purge **BEIDER** Caches vorab — neues `scripts/purge_ticker_cache_all.py` (1427 Docs) + `purge_edgar_cache_all.py` (547 Docs); der Ticker-Purge war **nicht optional**, weil `CachedYFinanceClient.get_ticker_info` bei warmem Cache das Dict zurückgibt **ohne** den rohen (gefixten) Client zu rufen → warmer Cache hätte den STUFE-A-Fix umgangen und genau die 5 UNCLEAR als falsch-resolved maskiert. Trigger via `gcloud run services proxy` + `scripts/trigger_cold_dry_run.py` (Dry-Run-Response trägt `yfinance_unresolved` direkt). **Konvergenz belegt:** `unresolved.count=5`, tickers **exakt** `{AMS.VI, RIGN.SW, ROL.L, SANO.HE, SCHA.OL}` (nicht mehr 0 — Maskierung tot); keiner der 13 Korrigierten in unresolved (ASM.AS sichtbar in EU-Basis); **Basis `edgar_filter: 687/695`** = 695 Basis-Passer (Audit-Rekonstruktion war 690@1349; +5 erklärt durch korrigierte Realfirmen), Restatement-Drops **8 bitgenau**, `going_concern_drops=0`, `data_source_error=2` (DGX/NBIX transiente EFTS-500 unter Cold-Load, Pass-Through). Arithmetik schließt: `1332 = 687 final + 632 basis-fail + 8 restatement + 5 unresolved + 0 GC`. **Offen (eigene Tickets, Stephans Calls):** 5 UNCLEAR via OpenFIGI/ISIN auflösen; **Financials-Auslöschung** (gross_margin≥30% droppt 103 Financials, 74 mit gm=0 — Metrik-, kein Qualitätsproblem); **revenue_growth≥0 auf TTM-Snapshot** brüchig (Nestlé −2.2 %, LVMH −4.7 % → Mehrjahres-Glättung erwägen); RO.SW-Roche illiquide Inhaber-Linie (liquide ROG.SW fehlt); periodischer Delisting-/Staleness-Sweep (Index-Drift); affirmativer GC-TRUE-Zweig in-Universum unbenutzt (akzeptiertes Residuum, Live-Beleg nur aus separatem Paid-Run, FRQN=True). Neue Ops-Skripte (`purge_ticker_cache_all.py`, `trigger_cold_dry_run.py`) sind working-tree/untracked. Memory: [[universe-completeness-degraded-dict-masking-closed]], [[prod-logging-dormant-and-cache-masks-verification]].

**Vier im 06-02-Dry-Run materialisierte Failure-Modi GEFIXT, DEPLOYED & im Cold-Run VERIFIZIERT 2026-06-03 — grün für den bezahlten Voll-Lauf.** Sequenzielle Sub-Phasen, STOP-Gate + Diff-Review hunk-für-hunk + eigener PR je Phase, sub-agent-getrieben (backend-developer), Claude Code ownte git/PR/Deploy. **Fundament — PR #16 „Log-Hygiene" (Merge `2719075`):** `configure_logging()` wurde **nie im FastAPI-Startup aufgerufen** → auf Cloud Run lief uvicorn mit Pythons last-resort-Handler (nur WARNING+, Plaintext) → **alle** `logger.info`-Aggregate (`basis_filter: N/M`, `edgar_skipped total`, Runner-Stage-Counts) + das designte CloudJsonFormatter-JSON (severity/trace) waren in Prod **still verschluckt**. Fix: `configure_logging()` im `app.main`-Import-Scope, **root=WARNING** (httpx loggt pro Request auf INFO → Flut-Vermeidung), **`app`-Logger=INFO** (propagierte Records am Root nicht refiltert). Im selben PR: **`has_active_enforcement` zu stummem No-op** (war 538 WARNING-Zeilen/Lauf reiner Stub-Spam → 0). **SUB-PHASE 1 — yfinance-Resolution (PR #17, Merge `5523121`):** 79 EU-Symbole lösten als 404 auf (lokal verifiziert: genuin kaputt/stale, nicht transient; `universe_cache` lieferte stale Daten und maskierte sie). Generator-Root-Cause `build_universe.py` (Trailing-Dot vor Suffix-Join: `BA..L`→`BA.L`, negativ-getestet `BRK.B`) + **`universe.json` 1389→1349** chirurgisch korrigiert (43 Name-belegte Renames inkl. Roche `ROG.SW→RO.SW`/Swiss Re/Ferrari/Sanofi; **−40 exakt auditiert: 0 unbeabsichtigte Drops** = 3 Delistings + 25 unresolved [Ticket-Seed, nicht geraten] + 12 Dedup/Kollaps) + **Resolution-Aggregat** (WARNING + `yfinance_unresolved` im Dry-Run-JSON = Dauer-Netz). **SUB-PHASE 2 — EFTS-Backoff (PR #18, Merge `fdf0ead`):** die 11 erschöpften Cold-Run-Skips waren **efts-500-Flakiness, kein Rate-403**; linear 2s/4s×3 → **exponentielles Full-Jitter `uniform(0, base·2^(n-1))`, base=1s, 5 Versuche** (Limiter+8/s unverändert, ist Min-Intervall-Pacer → kein Burst; 403 bleibt greppbar via Status-in-Message). **SUB-PHASE 4 — Going-Concern-Polarität (PR #19, Merge `a496dd5`, Rev `00083-55r`):** Boilerplate-Residuum GESCHLOSSEN — Diskriminator ist **Polarität/Assertion** (an echtem Filing-Text belegt), nicht Phrasen-Präsenz: gesunde Filer tragen „raise substantial doubt" nur als Pflicht (`whether`)/Negation (`no … raise`)/Hypothese (`may raise`); echte GC (FRQN) zusätzlich affirmativ „these factors raise …". Fix: In-Window-Primärform-Vorfilter behalten, dann Primärdoc fetchen (URL aus EFTS-`_id`) + `_has_affirmative_going_concern` (**klausel-scoped** Negation [nicht Token-Fenster — WTW/HIMS setzen `no` ~10+ Wörter vor `raise`], **∃ über alle Vorkommen**, schneller Regex-Strip statt bs4). **Cold-Integrations-Verifikation 2026-06-03 (547 Cache-Docs gepurgt, 28-min-Lauf, 200 OK, gegen Baseline 675/1389):** `going_concern_drops: []` (zweiseitig: {ADC,FR,HIMS,LIVN,WTW} weg ∧ kein neuer gesunder Name; **WTW war ein verdeckter EFTS-500-Skip → #2 und #4 gekoppelt**), `yfinance_unresolved: 0`, `data_source_error: 1` (COLM, 500 nicht 403, **behalten**), enforcement-Spam **0**, `basis_filter 690/1349` (EU 140/444, +14 zurückgeholt). **Neue Tickets (eigene Arbeit, nicht jetzt):** Enforcement-Screening (`tickets/2026-06-03-enforcement-screening.md`), ISIN-Anker+OpenFIGI (`tickets/2026-06-03-isin-canonical-anchor-openfigi.md`, Seed = die 25 unresolved), HTTP-Batch→Cloud-Run-Job (`tickets/2026-06-03-toolA-run-as-cloud-run-job.md` — der 28-min-Sync-Request belegt den Smell). Memory: [[toolA-going-concern-polarity-closed]], [[prod-logging-dormant-and-cache-masks-verification]]. **NEXT (Stephan, selbst): bezahlter Voll-Lauf.**
...
| 2026-06-12 | **TEMPORÄR:** `RepositoryRole:admin`-Bypass (id 5, `always`) im `main`-Ruleset (id 17168890), damit der Prod-Service direkt nach `main` pushen kann | Das 06-02-Ruleset (`pull_request`-Regel) blockte **alle** direkten Contents-API-Pushes des Service → 409 Conflict; betraf nicht nur `/selftest/push`, sondern den **Monats-Output-Push** (`run_monthly`, gleicher `push_file`→main) → wäre am 1.7. mitten im bezahlten Lauf zerbrochen. Self-Test (PR #33) fing es präventiv. Admin-Bypass = kleinster, reversibler, redeploy-freier Unblock; re-getestet grün (Sentinel 5e2dc98 auf origin/main) | **TECH-DEBT:** kein technisches Gate mehr für admin-authentifizierte Pushes auf `main` → „No-Push-ohne-Go"-Konvention gilt **verschärft**. Ziel-State nach 01.07: gescopter Bypass (GitHub-App **oder** Machine-User mit `RepositoryRole:maintain`). Ticket: `docs/superpowers/tickets/2026-06-12-scoped-bypass-main-ruleset.md` |
| 2026-08-20 | „Keine eigene Web-UI“ revidiert: Read-only-Viewer `app/viewer/` erlaubt | Die Dossiers waren nur in Obsidian lesbar; ein statischer Viewer macht sie unterwegs zugänglich und erlaubt erstmals den Quervergleich über Titel hinweg | Bricht eine dokumentierte Nicht-Ziel-Entscheidung. Gegengewicht: harte Grenze “nur Anzeige” — kein Backend, keine Eingabe, keine Berechnung; Obsidian bleibt führend |
| 2026-08-20 | Statt Spec-`SUPPRESSED_METRICS` eine **datierte** Defekt-Tabelle; fehlerhafte Werte bleiben sichtbar mit ⚠ | Eine handgepflegte Sperrliste ist eine Momentaufnahme des Wissensstands — sie hätte den erst beim Bau entdeckten vierten Fehler (Div-Yield-Glitch in 9 Dossiers) nicht enthalten. Ausblenden behauptet „gibt’s nicht“, obwohl der Wert im Dossier steht | Ein Eintrag markiert periodenbasiert, nicht wertbasiert: KO bekommt ein ⚠, obwohl sein Div-Yield stimmt. Bewusst — der Viewer darf nicht rechnen |
